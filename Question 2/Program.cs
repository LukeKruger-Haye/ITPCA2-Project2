...
namespace Project {
  class SoccerMS {
    ...
    void save_to_file() {
      string path = "player_information.txt"; 
      List<string> lines = new List<string>();

      foreach (Player player in players) {
        lines.Add(player.ToLine());
      }

      try {
        File.WriteAllLines(path, lines);
        Console.WriteLine("The player information has been printed to player_information.txt");
      } catch (Exception ex) {
    ...
    void menu_select(ushort user_input) {
      ...
      if (user_input == 5) save_to_file();
      if (user_input == 6) System.Environment.Exit(1); 
    }
